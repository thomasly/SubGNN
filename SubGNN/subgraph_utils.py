...
def calc_f1(logits, labels, avg_type="macro", multilabel_binarizer=None):
    ...
    specified logits and labelss
    """
    if multilabel_binarizer is not None:  # multi-label prediction
        # perform a sigmoid on each logit separately & use > 0.5 threshold to make
        # prediction
        ...
        pred = torch.argmax(
            logits, dim=-1
        )  # get predictions by finding the indices with max logits
        score = f1_score(labels.cpu().detach(), pred.cpu().detach(), average=avg_type)
...
def get_border_nodes(graph, subgraph):
    """
    Returns (1) an array containing the border nodes of the subgraph (i.e. all nodes
    that have an edge to a node not in the subgraph, but are themselves in the subgraph)
    and (2) an array containing all of the nodes in the base graph that aren't in the
    subgraph
    """

    # get all of the nodes in the base graph that are not in the subgraph
    non_subgraph_nodes = np.array(
        list(set(graph.nodes()).difference(set(subgraph.nodes())))
    )
    ...
    border_A = A[
        np.ix_(subgraph_nodes - 1, non_subgraph_nodes - 1)
    ]  # NOTE: Need to subtract 1 bc nodes are indexed starting at 1

    # the nodes in the subgraph are border nodes if they have at least one edge to a
...
def get_component_border_neighborhood_set(
    networkx_graph, component, k, ego_graph_dict=None
):
    """
    ...
    ego_graph_dict: dictionary mapping from node id to precomputed ego_graph for the
    node
    """
    ...
    # calculate the ego graph for each node in the connected component & take the union
    # of all nodes
    neighborhood = set()
    for node in component_set:
        # if it hasn't already been computed, calculate the ego graph (i.e. induced
        # subgraph of neighbors centered at node with specified radius)
        if ego_graph_dict is None:
            ego_g = nx.ego_graph(networkx_graph, node, radius=k).nodes()
        # NOTE: nodes in dict were indexed with 0, while our nodes are indexed starting
        # at 1
        else:
    ...
    # remove from the unioned ego sets all nodes that are actually in the component this
    # will leave only the nodes that are in the k-hop border, but not in the subgraph
    # component
...
def weighted_sum(matrix: torch.Tensor, attention: torch.Tensor) -> torch.Tensor:
    """
    Takes a matrix of vectors and a set of weights over the rows in the matrix (which we
    call an "attention" vector), and returns a weighted sum of the rows in the matrix.
    This is the typical computation performed after an attention mechanism. Note that
    while we call this a "matrix" of vectors and an attention "vector", we also handle
    higher-order tensors.  We always sum over the second-to-last dimension of the
    "matrix", and we assume that all dimensions in the "matrix" prior to the last
    dimension are matched in the "vector".  Non-matched dimensions in the "vector" must
    be `directly after the batch dimension`. For example, say I have a "matrix" with
    dimensions `(batch_size, num_queries, num_words, embedding_dim)`.  The attention
    "vector" then must have at least those dimensions, and could have more. Both:
        - `(batch_size, num_queries, num_words)` (distribution over words for each
        query)
        - `(batch_size, num_documents, num_queries, num_words)` (distribution over words
        in a query for each document)
    are valid input "vectors", producing tensors of shape:
    `(batch_size, num_queries, embedding_dim)` and
    `(batch_size, num_documents, num_queries, embedding_dim)` respectively.
    """
    # We'll special-case a few settings here, where there are efficient (but
    # poorly-named) operations in pytorch that already do the computation we need.

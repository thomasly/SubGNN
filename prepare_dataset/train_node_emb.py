...
if config.MINIBATCH == "GraphSaint":
...
if config.MINIBATCH == "NeighborSampler":
    all_hyperparameters = {
        "batch_size": config.POSSIBLE_BATCH_SIZES,
        "hidden": config.POSSIBLE_HIDDEN,
        "output": config.POSSIBLE_OUTPUT,
        "lr": config.POSSIBLE_LR,
        "wd": config.POSSIBLE_WD,
        "nb_size": config.POSSIBLE_NB_SIZE,
        "dropout": config.POSSIBLE_DROPOUT,
    }
    curr_hyperparameters = {
        "batch_size": config.POSSIBLE_BATCH_SIZES[0],
        "hidden": config.POSSIBLE_HIDDEN[0],
        "output": config.POSSIBLE_OUTPUT[0],
        "lr": config.POSSIBLE_LR[0],
        "wd": config.POSSIBLE_WD[0],
        "nb_size": config.POSSIBLE_NB_SIZE[0],
        "dropout": config.POSSIBLE_DROPOUT[0],
    }
    ...
    all_hyperparameters = {
        ...
        "lr": config.POSSIBLE_LR,
        "wd": config.POSSIBLE_WD,
        "walk_length": config.POSSIBLE_WALK_LENGTH,
        "num_steps": config.POSSIBLE_NUM_STEPS,
        "dropout": config.POSSIBLE_DROPOUT,
    }
    curr_hyperparameters = {
        "batch_size": config.POSSIBLE_BATCH_SIZES[0],
        ...
        "lr": config.POSSIBLE_LR[0],
        "wd": config.POSSIBLE_WD[0],
        "walk_length": config.POSSIBLE_WALK_LENGTH[0],
        "num_steps": config.POSSIBLE_NUM_STEPS[0],
        "dropout": config.POSSIBLE_DROPOUT[0],
    }
...
def train(epoch, model, optimizer):
    ...
    global all_data, best_val_acc, best_embeddings, best_model, curr_hyperparameters
    ...
    for data in loader:
        ...
        data.y[: curr_train_pos.size(1)] = 1.0
        ...
        data.y[: curr_val_pos.size(1)] = 1.0
    ...
    res = "\t".join(
        [
            "Epoch: %04d" % (epoch + 1),
            "train_loss = {:.5f}".format(total_loss),
...
def generate_grid(all_hyperparameters, current_params={}):
    if len(all_hyperparameters) == 0:
        yield current_params
    else:
        key = list(all_hyperparameters.keys())[0]
        new_param_dict = all_hyperparameters.copy()
        new_param_dict.pop(key)
        for param in all_hyperparameters[key]:
            current_params[key] = param
            for cp in generate_grid(new_param_dict, current_params):
                yield cp


def generate_emb():

    global all_data, best_embeddings, best_model, all_hyperparameters
    global best_hyperparameters, all_losses, device
    # global curr_hyperparameters

    all_data = preprocess.read_graphs(str(config.DATASET_DIR / "edge_list.txt"))

    # Iterate through hyperparameter type (shuffled)
    # shuffled_param_type = random.sample(
    #     all_hyperparameters.keys(), len(all_hyperparameters.keys())
    # )
    # shuffled_param_type = all_hyperparameters.keys()
    for curr_hyperparameters in generate_grid(all_hyperparameters):
        ...
        model = mdl.TrainNet(
            ...
            config.CONV.lower().split("_")[1],
            curr_hyperparameters["dropout"],
        ).to(device)
        ...
        all_losses[
            ";".join([str(v) for v in curr_hyperparameters.values()])
        ] = curr_losses
    ...
    # Save best embeddings
    torch.save(
        best_embeddings, config.DATASET_DIR / (config.CONV.lower() + "_embeddings.pth")
    )

...
def calc_loss_both(data, dot_pred):
    ...
    Args
        ...
        - dot_pred (tensor long, shape=(nodes, classes)): predictions calculated from
            dot product
...
def el_dot(embed, edges, test=False):
    ...
    dots = torch.bmm(
        source.view(edges.shape[1], 1, embed.shape[1]),
        target.view(edges.shape[1], embed.shape[1], 1),
    )
...
def calc_roc_score(
    ...
    if multi_class:
        if save_plots != "":
            class_roc, class_ap, class_f1 = plot_roc_ap(
                true_all,
                pred_all,
                save_plots,
                loss=loss,
        ...
        if save_plots != "":
            return roc_auc, acc, f1_micro, class_roc, class_ap, class_f1
        ...
        true_all = (
            torch.cat((torch.ones(len(pred_pos)), torch.zeros(len(pred_neg))), 0)
            .cpu()
            .detach()
            .numpy()
        )
        ...
        if save_plots != "":
            plot_roc_ap(true_all, pred_all, save_plots, loss, multilabel=multilabel)
        ...
        if len(labels) > 0:  # Multiclass classification
            for c in range(y_true.shape[1]):
                ...
                plt.plot(
                    fpr[c],
                    tpr[c],
                    label=str(labels[c]) + " (area = {:.5f})".format(roc[c]),
                )
        ...
        if len(labels) > 0:  # Multiclass classification
            for c in range(y_true.shape[1]):
                ...
                plt.plot(
                    recall[c],
                    precision[c],
                    label=str(labels[c]) + " (area = {:.5f})".format(ap[c]),
                )

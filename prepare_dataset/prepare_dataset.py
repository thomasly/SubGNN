...
class SyntheticGraph:
    ...
    def initialize_features(self, **kwargs):
        ...
        if self.features_type == "one_hot":
            ...
            raise Exception(
                "The feature initialization you specified is not implemented"
            )
    ...
    def generate_and_add_subgraphs(self, **kwargs):
        ...
        Return
            - subgraphs (list of lists): list of subgraphs, where each subgraph is a
              list of nodes
        """
        ...
        if self.subgraph_type == "random":
            subgraphs = self._get_subgraphs_randomly(
                n_subgraphs, n_nodes_in_subgraph, **kwargs
            )
        elif self.subgraph_type == "bfs":
            subgraphs = self._get_subgraphs_by_bfs(
                n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
            )
        elif self.subgraph_type == "staple":
            subgraphs = self._get_subgraphs_by_k_hops(
                n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
            )
        elif self.subgraph_type == "plant":
            if desired_property == "coreness":
                subgraphs = self._get_subgraphs_by_coreness(
                    n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
                )
            else:
                subgraphs = self._get_subgraphs_by_planting(
                    n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
                )
        else:
    ...
    def staple_component_to_graph(self, n_nodes_in_subgraph, graph_root_node, **kwargs):
        ...
        Args
            - n_nodes_in_subgraph (int): number of nodes in each subgraph
            - graph_root_node (int): node in the base graph that the component should
              be "stapled" to

        Return
            - cc_node_ids (list): nodes in a connected component
            - cc_root_node (int): node in the connected component (subgraph) to connect
              with the graph_root_node
        """
        ...
        cc_node_ids = list(
            range(len(self.graph.nodes), len(self.graph.nodes) + n_nodes_in_subgraph)
        )
    ...
    def _get_subgraphs_by_k_hops(
        self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
    ):
        """
        Generate subgraphs that are k hops apart, staple each subgraph to the base graph
        by adding edge between a random node from the subgraph and a random node from
        the base graph

        Args
            - n_subgraphs (int): number of subgraphs
            - n_nodes_in_subgraph (int): number of nodes in each subgraph
            - n_connected_components (int): number of connected components in each
              subgraph

        Return
            - validated_subgraphs (list of lists): list of subgraphs, where each
              subgraph is a list of nodes
        """
        ...
        shuffle_cc = False
        if n_connected_components is None:
            shuffle_cc = True
        ...
        for s in range(n_subgraphs):
            ...
            cc_node_ids, cc_root_node = self.staple_component_to_graph(
                n_nodes_in_subgraph, graph_root_node, **kwargs
            )
            curr_subgraph.extend(cc_node_ids)
            seen_nodes.extend(cc_node_ids)
            all_cc_start_nodes.append(
                cc_root_node
            )  # keep track of start nodes across CCs

            # Get nodes that are k hops away
            n_hops_paths = nx.single_source_shortest_path_length(
                self.graph, graph_root_node, cutoff=k_hops
            )
            candidate_nodes = [
                node
                for node in n_hops_paths
                if self.is_k_hops_from_all_cc(node, all_cc_start_nodes, k_hops)
            ...
            if len(candidate_nodes) == 0:
                candidate_nodes = [
                    node
                    for node, length in n_hops_paths.items()
                    if length == max(n_hops_paths.values())
                ]
            if shuffle_cc:
                n_connected_components = random.sample(cc_range, 1)[0]

            for c in range(n_connected_components - 1):
                new_graph_root_node = random.sample(candidate_nodes, 1)[
                    0
                ]  # choose a random node that is k hops away
                seen_nodes.append(new_graph_root_node)
                cc_node_ids, cc_root_node = self.staple_component_to_graph(
                    n_nodes_in_subgraph, new_graph_root_node, **kwargs
                )
                curr_subgraph.extend(cc_node_ids)
                seen_nodes.extend(cc_node_ids)
                all_cc_start_nodes.append(
            ...
            if len(curr_subgraph) >= n_nodes_in_subgraph * n_connected_components:
                actual_num_cc = nx.number_connected_components(
                    self.graph.subgraph(curr_subgraph)
                )
                if shuffle_cc and actual_num_cc in config.CC_RANGE:
                    subgraphs.append(curr_subgraph)
                elif not shuffle_cc and actual_num_cc > 1:
                    subgraphs.append(curr_subgraph)  # must have >1 CC
        ...
        for s in subgraphs:
            ...
            if shuffle_cc and actual_num_cc in config.CC_RANGE:
                validated_subgraphs.append(s)
            elif not shuffle_cc and actual_num_cc > 1:
                validated_subgraphs.append(s)  # must have >1 CC
        print(len(validated_subgraphs))
        return validated_subgraphs

    def _get_subgraphs_by_coreness(
        self,
        n_subgraphs,
        n_nodes_in_subgraph,
        ...
        **kwargs
        ...
        """
        Sample nodes from the base graph that have at least n nodes with k core. Merge
          the edges from the generated
        subgraph with the edges from the base graph. Optionally, remove all other edges
          in the subgraphs
        ...
        Args
            ...
            - n_connected_components (int): number of connected components in each
              subgraph
            - remove_edges (bool): true if should remove unmerged edges in subgraphs,
              false otherwise
        ...
        for n, k in k_core_dict.items():
            if k in nodes_with_core_number:
                nodes_with_core_number[k].append(n)
            else:
                nodes_with_core_number[k] = [n]
        ...
        for k in nodes_with_core_number:
            ...
            for s in range(n_subgraphs):
                ...
                for c in range(n_connected_components):
                    if len(nodes_with_k_cores) < n_nodes_in_subgraph:
                        break

                    con_component = self.generate_subgraph(
                        n_nodes_in_subgraph, **kwargs
                    )
                    cc_node_ids = random.sample(nodes_with_k_cores, n_nodes_in_subgraph)

                    # Relabel subgraph to the same ids as the randomly sampled nodes
                    cc_id_mapping = {
                        curr_id: new_id
                        for curr_id, new_id in zip(con_component.nodes, cc_node_ids)
                    }
                    nx.relabel_nodes(con_component, cc_id_mapping, copy=False)

                    if remove_edges:
                        # Remove the existing edges between nodes in the planted
                        # subgraph (except the ones to be added)
                        self.graph.remove_edges_from(
                            self.graph.subgraph(cc_node_ids).edges
                        )

                    # Combine the base graph & subgraph.
                    # Nodes with the same ID are merged

                    # NOTE: attributes from subgraph take precedent over attributes from
                    # self.graph
                    joined_graph = nx.compose(self.graph, con_component)
                    self.graph = joined_graph.copy()

                    curr_subgraph.extend(cc_node_ids)  # add nodes to subgraph
                    nodes_with_k_cores = list(
                        set(nodes_with_k_cores).difference(set(cc_node_ids))
                    )
    ...
    def _get_subgraphs_by_bfs(
        self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
    ):
        """
        Sample n_connected_components number of start nodes from the base graph. Perform
        BFS to create subgraphs of size n_nodes_in_subgraph.
        ...
        for s in range(n_subgraphs):
            ...
            for start_node in start_nodes:
                ...
                nodes = nodes[
                    :n_nodes_in_subgraph
                ]  # limit nodes to n_nodes_in_subgraph

                if max(nodes) > max(self.graph.nodes):
                    print(max(nodes), max(self.graph.nodes))
                assert max(nodes) <= max(self.graph.nodes)

                assert nx.is_connected(
                    self.graph.subgraph(nodes)
                )  # check to see if selected nodes represent a conencted component
    ...
    def generate_subgraph(self, n_nodes_in_subgraph, **kwargs):
        ...
        if subgraph_generator == "cycle":
            ...
            p = kwargs.get("p", 0.5)
            q = kwargs.get("q", 0)
            G = extended_barabasi_albert_graph(
                n_nodes_in_subgraph, m, p, q, seed=config.RANDOM_SEED
            )
    ...
    def is_k_hops_from_all_cc(self, cand, all_cc_start_nodes, k_hops):
        ...
        Return
            - True if the candidate node is k hops away from all CC start nodes, false
              otherwise
        """
    ...
    def _get_subgraphs_by_stapling(
        self, n_subgraphs, n_nodes_in_subgraph, n_connected_components, **kwargs
    ):
        """
        Generate n subgraphs, staple each subgraph to the base graph by adding an edge
        between random node from the subgraph and a random node from the base graph.
        ...
        for s in range(n_subgraphs):
            ...
            for c in range(n_connected_components):
                ...
                if c > 0 and k_hops != -1:
                    # make sure to sample the next node k hops away from the previously
                    # sampled root node and check to see that the selected start node is
                    # k hops away from all previous start nodes
                    n_hops_paths = nx.single_source_shortest_path_length(
                        self.graph, cc_root_node, cutoff=k_hops
                    )
                    ...
                    candidate_nodes = [
                        ...
                        if self.is_k_hops_from_all_cc(cand, all_cc_start_nodes, k_hops)
                    ]
                    if len(candidate_nodes) == 0:
                        raise Exception(
                            "There are no nodes that are k hops away from all other CC "
                            "start nodes."
                        )
                    ...
                    k_core_dict = nx.core_number(self.graph)
                    nodes_with_core_number = [
                        node
                        for node, core_num in k_core_dict.items()
                        if core_num == k_core_to_sample
                    ]
                    cc_root_node = random.sample(nodes_with_core_number, 1)[0]
                    all_cc_start_nodes.append(
                        cc_root_node
                    )  # keep track of start nodes across CCs
                else:  # if we're not trying to sample each CC k hops away OR if it's
                    # the first time we sample a CC, just randomly sample a start node
                    # from the graph randomly sample root node where the CC will be
                    # attached
                    cc_node_ids = list(
                        range(
    ...
    def _get_subgraphs_by_planting(
        self,
        n_subgraphs,
        n_nodes_in_subgraph,
        n_connected_components,
        ...
        **kwargs
        ...
        """
        Randomly sample nodes from base graph that will be in each subgraph. Merge the
        edges from the generated subgraph with the edges from the base graph.
        Optionally, remove all other edges in the subgraphs
        ...
        for s in range(n_subgraphs):
            ...
            for c in range(n_connected_components):
                ...
                if k_core_to_sample != -1:
                    ...
                    cc_node_ids = random.sample(
                        nodes_with_core_number, n_nodes_in_subgraph
                    )
                else:
                ...
                if remove_edges:
                    # remove the existing edges between nodes in the planted subgraph
                    # (except the ones to be added)
                ...
                joined_graph = nx.compose(
                    self.graph, con_component
                )  # NOTE: attributes from subgraph take precedent over attributes from
                # self.graph
    ...
    def _get_property(self, subgraph, subgraph_property):
        ...
        if subgraph_property == "density":
            ...
            nodes_except_subgraph = set(self.graph.nodes).difference(
                set(subgraph.nodes)
            )
            n_boundary_edges = len(
                list(
                    nx.edge_boundary(self.graph, subgraph.nodes, nodes_except_subgraph)
                )
            )
        ...
        for s in subgraphs:
            ...
            if desired_property == "density":
                ...
                while True:
                    ...
                    if (
                        abs(curr_subg_property - desired_prop_value)
                        < config.DENSITY_EPSILON
                    ):
                        break
                    if n_tries >= config.MAX_TRIES:
                        break

                    if (
                        curr_subg_property > desired_prop_value
                    ):  # remove edges to decrease density
            ...
            # CUT RATIO
                ...
                while True:
                    ...
                    subgraph_boundary_edges = list(
                        nx.edge_boundary(
                            self.graph, subgraph.nodes, nodes_except_subgraph
                        )
                    )

                    if (
                        curr_subg_property > desired_prop_value
                    ):  # high cut ratio -> too many edges
                        ...
                        sampled_rest_graph_node = random.sample(
                            nodes_except_subgraph, 1
                        )[0]
                        self.graph.add_edge(
                            sampled_subgraph_node, sampled_rest_graph_node
                        )
    ...
    def _relabel_nodes(self, subgraphs, **kwargs):
        """
        Relabel nodes in the graph and subgraphs to ensure that all nodes are indexed
        consecutively
        """
        largest_cc = max(nx.connected_components(self.graph), key=len)
        removed_nodes = set(list(self.graph.nodes)).difference(set(largest_cc))
        print(
            "Original graph: %d, Largest cc: %d, Removed nodes: %d"
            % (len(self.graph.nodes), len(largest_cc), len(removed_nodes))
        )
        self.graph = self.graph.subgraph(largest_cc)
        mapping = {
            k: v for k, v in zip(list(self.graph.nodes), range(len(self.graph.nodes)))
        }
...
def write_f(sub_f, sub_G, sub_G_label, mask):
    """
    Write subgraph information into the appropriate format for SubGNN (tab-delimited
    file where each row has dash-delimited nodes, subgraph label, and train/val/test
    label).

    Args
        - sub_f (str): file directory to save subgraph information
        - sub_G (list of lists): list of subgraphs, where each subgraph is a list of
          nodes
...
def main():
    if config.GENERATE_SYNTHETIC_G:
        synthetic_graph = SyntheticGraph(
            base_graph_type=config.BASE_GRAPH_TYPE,
        ...
        nx.write_edgelist(
            synthetic_graph.graph, str(config.DATASET_DIR / "edge_list.txt"), data=False
        )
    ...
    if config.GENERATE_NODE_EMB:
        train_node_emb.generate_emb()
